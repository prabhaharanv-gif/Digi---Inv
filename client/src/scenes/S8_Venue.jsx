  ...
  return (
    <div style={{ width:'100%', height:'100%', overflow:'hidden', position:'relative', background:'linear-gradient(160deg,#cce5c2 0%,#a8c89e 40%,#8ab87e 100%)', display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:0.7}}
        ...
        <div style={{ fontFamily:"'Cinzel',serif", color:'#4a2000', fontWeight:700, fontSize:'clamp(0.45rem,1.4vw,0.55rem)', letterSpacing:'5px' }}>✦ &nbsp; THE VENUE &nbsp; ✦</div>
        <div style={{ fontFamily:"'Great Vibes',cursive", fontSize:'clamp(2rem,6.5vw,3.5rem)', color:'#4a2000', marginTop:'0.3rem', lineHeight:1.1, textShadow:'0 1px 8px rgba(44,74,40,0.1)' }}>
          {event?.venue}
        </div>
        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontStyle:'italic', fontSize:'clamp(0.82rem,2.3vw,1rem)', color:'#3d1a00', marginTop:'0.5rem', lineHeight:1.7 }}>
          {event?.venueAddress}
        </div>
        <GoldDiv />
          ...
          {INFO.map((item,i) => (
            <motion.div key={item.label} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:0.2+i*0.08,duration:0.5}}>
              <div style={{ background:'rgba(255,255,255,0.55)', backdropFilter:'blur(4px)', border:'1px solid rgba(201,146,42,0.22)', borderRadius:8, padding:'clamp(0.6rem,2vw,0.8rem)', boxShadow:'0 2px 10px rgba(44,74,40,0.06)' }}>
                ...
                <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(0.75rem,2.2vw,0.88rem)', color:'#4a2000' }}>{item.val}</div>
              </div>
            </motion.div>
          ))}
        </div>
        <a href={event?.venueMaps||'#'} target="_blank" rel="noreferrer" style={{
          ...
          background:'linear-gradient(135deg,#5c2800,#8b4513)', color:'white',
